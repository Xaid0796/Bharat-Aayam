// Configuration & GNews Integration
const API_KEY = 'f5017e957c1d9daead58501cec910233';
const CACHE_EXPIRATION_MS = 15 * 60 * 1000; // 15 minutes local storage cache
...
function getNewsUrl(category, page = 1) {
  const langParam = currentLang; // matches 'en' or 'hi'
  if (category === 'politics') {
    return `https://gnews.io/api/v4/search?q=politics&country=in&lang=${langParam}&apikey=${API_KEY}`;
  }
  if (category === 'world') {
    return `https://gnews.io/api/v4/top-headlines?category=world&lang=${langParam}&apikey=${API_KEY}`;
  }
  if (category === 'national') {
    return `https://gnews.io/api/v4/top-headlines?category=nation&country=in&lang=${langParam}&page=${page}&apikey=${API_KEY}`;
  }
  ...
  const gCat = gnewsCategories[category] || 'general';
  return `https://gnews.io/api/v4/top-headlines?category=${gCat}&country=in&lang=${langParam}&apikey=${API_KEY}`;
}
...
async function loadNews(category) {
  ...
  const cacheKey = `bp_gnews_data_${category}_${currentLang}`;
  const cacheTimeKey = `bp_gnews_time_${category}_${currentLang}`;
  const cachedData = localStorage.getItem(cacheKey);
  const cachedTime = localStorage.getItem(cacheTimeKey);
  const now = Date.now();

  // If fresh API cache exists, use it (no new API call needed yet)
  if (cachedData && cachedTime && (now - parseInt(cachedTime) < CACHE_EXPIRATION_MS)) {
  ...
  try {
    ...
    if (!response.ok) throw new Error('API server returned response error.');
    
    const data = await response.json();
    if (!data.articles || data.articles.length === 0) throw new Error('Empty news collection fetched.');

    // ── PERMANENT ARCHIVE: merge new articles in, never delete old ones ──
    mergeIntoArchive(data.articles, category);

    // Get the latest stories for this category from the permanent archive
    const categoryArchive = loadArchive().filter(a => a.ba_category === category);
    loadedArticles = categoryArchive.slice(0, HOMEPAGE_LIMIT);
    
    // Update short-term API cache (still useful for fast loads / offline)
    try {
      localStorage.setItem(cacheKey, JSON.stringify(loadedArticles));
    ...
    // Fallback to reading from permanent archive first
    const categoryArchive = loadArchive().filter(a => a.ba_category === category);
    if (categoryArchive.length > 0) {
      ...
      // If archive is empty, fallback to cachedData or oldCache
      if (cachedData) {
        loadedArticles = JSON.parse(cachedData).slice(0, HOMEPAGE_LIMIT);
        showToast(translations[currentLang].offlineModeToast);
        applyFilters();
      } else {
        const oldCache = localStorage.getItem(`bp_cache_${category}_${currentLang}`);
        if (oldCache) {
          loadedArticles = JSON.parse(oldCache).slice(0, HOMEPAGE_LIMIT);
          showToast(translations[currentLang].offlineBackupToast);
          applyFilters();
        } else {
...
// Load additional articles for Infinite Scroll (Trending section only)
async function loadMoreTrending() {
  if (isLoadingMore || !hasMoreTrending || currentCategory !== 'national' || isShowingBookmarks) return;

  isLoadingMore = true;
  appendInfiniteLoader();

  trendingPage++;

  const langParam = currentLang;
  const cacheKey = `bp_gnews_data_national_page_${trendingPage}_${langParam}`;
  const cacheTimeKey = `bp_gnews_time_national_page_${trendingPage}_${langParam}`;
  const cachedData = localStorage.getItem(cacheKey);
  const cachedTime = localStorage.getItem(cacheTimeKey);
  const now = Date.now();
  ...
  if (cachedData && cachedTime && (now - parseInt(cachedTime) < CACHE_EXPIRATION_MS)) {
    ...
    const url = getNewsUrl('national', trendingPage);
    try {
      const response = await fetch(url);
      if (!response.ok) {
  ...
  if (newArticles.length > 0) {
    loadedArticles = [...loadedArticles, ...newArticles];
    displayedArticles = [...displayedArticles, ...newArticles];
    renderNews(displayedArticles);
  } else {
    hasMoreTrending = false;
    // Hide button if truly no more data
    if (displayedArticles.length <= currentBatchSize) {
